...
const chapter4 = [
  {
    ...
    successMessage: '<code>.filter()</code> always returns a new array. If nothing passes the test, you get an empty array <code>[]</code>, never <code>null</code> or <code>undefined</code>. The original array is never modified.',
  ...
  },
  {
    ...
    beltAfterLabel: 'upper belt',
    successMessage: '<code>.map()</code> always returns an array with the same length as the input. Each element is transformed by your function. Need to both transform and remove items? Chain <code>.filter()</code> before <code>.map()</code>.',
  ...
  },
  {
    ...
    successMessage: 'Without a second argument, <code>.reduce()</code> uses the first array element as the starting value and begins from the second. With an explicit starting value, it processes every element.',
  ...
  },
  {
    ...
    beltBefore: ['salmon', 'tuna', 'shrimp'],
    beltAfter: ['salmon', 'tuna', 'shrimp'],
    successMessage: 'While <code>.length</code> gives you a total count, <code>.reduce()</code> can count conditionally, sum up prices, group items into categories, or build entirely new data structures from an array.',
  ...
  },
  {
    ...
    successMessage: 'Dot notation (<code>obj.key</code>) is cleaner, but bracket notation (<code>obj["key"]</code>) lets you use variables: <code>const k = "price"</code> then <code>obj[k]</code>. Brackets also handle keys with spaces or special characters.',
  ...
  },
  {
    ...
    successMessage: '<code>.filter()</code> returns a new array and never changes the original. In real apps, it powers search boxes, role-based access, data dashboards, and any feature that shows a subset of data.',
    secondBeltAfterLabel: 'lower belt',
  },
  {
    ...
    beltAfterLabel: 'upper belt',
    successMessage: 'Extracting one property from each object into a flat array is called "plucking." It\'s one of the most common <code>.map()</code> patterns, especially when working with API responses or database results.',
