...
const chapter4 = [
  {
    ...
    successMessage: 'Look at the two belts: the upper belt still has all 4 dishes, but the lower belt only has the salmon. <code>.filter()</code> always creates a new array and leaves the original untouched.',
  ...
  },
  {
    ...
    beltAfterLabel: 'upper belt',
    successMessage: 'See the lower belt? Every dish got the wasabi treatment! <code>.map()</code> always returns an array with the same length as the input. Each element is transformed by your function, and the original stays untouched above.',
  ...
  },
  {
    ...
    successMessage: 'Three dishes on the belt became one menu string on the plate! <code>.reduce()</code> combines all items step by step. Without a second argument, it uses the first element as the starting value and begins from the second.',
  ...
  },
  {
    ...
    beltBefore: ['salmon', 'tuna', 'shrimp'],
    beltAfter: ['salmon', 'tuna', 'shrimp'],
    successMessage: 'See the 3 on the plate? Your function visited each dish and added 1 each time. The <code>0</code> you passed as the second argument was the starting count. Without it, <code>.reduce()</code> would use the first dish as the starting value, which would be a string!',
  ...
  },
  {
    ...
    successMessage: 'The price tag says 5! You used <code>belt[0]</code> to grab the first dish, then <code>.price</code> to read its property. You can chain like this because <code>belt[0]</code> returns the object, and the dot digs into it.',
  ...
  },
  {
    ...
    successMessage: 'Only the tuna and shrimp made it to the lower belt, the budget-friendly picks! Your arrow function checked <code>dish.price</code> on each object. <code>.filter()</code> works the same whether the items are strings or objects.',
    secondBeltAfterLabel: 'lower belt',
  },
  {
    ...
    beltAfterLabel: 'upper belt',
    successMessage: 'Look at the lower belt: objects with names and prices went in, but only the name strings came out! Pulling one property from each object is called "plucking" and it\'s one of the most common <code>.map()</code> patterns.',
